...
plm, tokenizer, model_config, WrapperClass = load_plm("roberta",
                                                      "roberta-large")

# load generation model for template generation
template_generate_model,\
    template_generate_tokenizer,\
    template_generate_model_config,\
    template_tokenizer_wrapper = load_plm('t5', 't5-base')
...
def evaluate(model, val_dataloader, save=False):
    model.eval()
    allpreds = []
    alllabels = []
    with torch.no_grad():
...
if auto_t:
    ...
    # beam_width is set to 5 here for efficiency, to improve performance,
    # try a larger number.
    ...
    template_texts = [
        template_generator.convert_template(template_text, original_template)
